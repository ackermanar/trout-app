...
server <- function(input, output, session) { # nolint
    observe({
        ...
        candidate_file <- input$candidate_file
        candidates <- read_table(candidate_file$datapath)
        ...
        raw_ped <- read_table(pedigree_file$datapath) %>%
    ...
    if (!is.null(input$weight_file) && !is.null(input$length_file)) {
        #read in EBV files
        weight_file <- input$weight_file
        req(weight_file)
        weight_ebvs <- read_table(weight_file$datapath)

        length_file <- input$length_file
        req(length_file)
        length_ebvs <- read_table(length_file$datapath)
    ...
    bindEvent(input$pedigree_file, input$candidate_file, input$download, input$weight_file, input$length_file)
}
